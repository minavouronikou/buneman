...
#ifdef MKL
/**************************************************** MKL Matrices ********************************************************************/
int solver_system(my_float *vectorX,my_float *tableA,int rowsA,int colsA,int colsX ,int *p){
	
	int info;
	int status;
	const char trans='N';
	my_float *tableB= (my_float*)mkl_malloc( sizeof(my_float)*(rowsA*colsA), 128);
	memcpy(tableB,tableA,rowsA*colsA*sizeof(my_float));
	
#ifdef sprecision
	
	sgetrf(&rowsA,&rowsA,tableB,&rowsA,p,&status);
	sgetrs (&trans , &rowsA , &colsX , tableB , &rowsA , p , vectorX , &colsA , &info);
