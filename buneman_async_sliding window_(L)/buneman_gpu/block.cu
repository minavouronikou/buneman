...
*   Master thesis
*   In this version the teqnique of "sliding window" is used to run bigger problems
*/
...
int main(int argc , char *argv[]){
	...
	cudaMalloc((void **) &d_P, 2*n*q*sizeof(my_float));
	cudaMalloc((void **) &d_Q, 2*n*q*sizeof(my_float));
	...
	cudaStreamCreate(&stream[2]);cudaStreamCreate(&stream[3]);

	cudaMemcpy(&d_Q[i3to1(0,0,0,n,q)], &Q[i3to1(0,0,0,n,q)], n*q*sizeof(my_float), cudaMemcpyHostToDevice);
	cudaMemcpy(&d_P[i3to1(0,0,0,n,q)], &P[i3to1(0,0,0,n,q)], n*q*sizeof(my_float), cudaMemcpyHostToDevice);
	...
	for(k=1;k<=jq-1;k++){
		...
		for(j=0;j<pow(2,jq-k)-1;j++){
			...
			vector_addition(d_tempVector[0],&d_P[i3to1(buffsw,idx1-idx2,0,n,q)],&d_P[i3to1(buffsw,idx1+idx2,0,n,q)],q,q,1,1);
			matrixVector_multiplication(d_tempVector[1],d_matrixT,d_tempVector[0],q,q,q,1.0);
			cudaStreamSynchronize(stream[2]);
			vector_addition(d_tempVector[0],d_tempVector[1],&d_Q[i3to1(buffsw,idx1,0,0,q)],q,q,1,-1);
			matrixVector_multiplication(d_tempVector[1],d_matrixB,d_tempVector[0],q,q,q,1.0);
			vector_addition(&d_P[i3to1(!buffsw,idx1,0,n,q)],&d_P[i3to1(buffsw,idx1,0,n,q)],d_tempVector[1],q,q,1,-1);
			matrixVector_multiplication(d_tempVector[1],d_matrixT,&d_P[i3to1(!buffsw,idx1,0,n,q)],q,q,q,2.0);
			
			cublasSetStream(handle, stream[1]);
			vector_addition(d_tempVector[0],&d_Q[i3to1(buffsw,idx1-idx2,0,n,q)],&d_Q[i3to1(buffsw,idx1+idx2,0,n,q)],q,q,1,1);
			cudaStreamSynchronize(stream[0]);
			vector_addition(d_tempVector[0],d_tempVector[0],d_tempVector[1],q,q,1,-1);
			matrixVector_multiplication(&d_Q[i3to1(!buffsw,idx1,0,n,q)],d_matrixT,d_tempVector[0],q,q,q,1.0);
			
		
		}
		cudaStreamSynchronize(stream[1]);
		cudaMemcpyAsync(&Q[i3to1(k,0,0,n,q)],&d_Q[i3to1(!buffsw,0,0,n,q)],n*q*sizeof(my_float),cudaMemcpyDeviceToHost,stream[2]);
		cudaMemcpyAsync(&P[i3to1(k,0,0,n,q)],&d_P[i3to1(!buffsw,0,0,n,q)],n*q*sizeof(my_float),cudaMemcpyDeviceToHost,stream[3]);
		buffsw = !buffsw;
	}
	...
	calcInvB(d_matrixB,d_B,d_T,q,k-1,multiply,ipiv,d_tempMatrix[0]);
	matrixVector_multiplication(d_tempVector[0],d_matrixB,&d_Q[i3to1(buffsw,idx1,0,n,q)],q,q,q,1.0);
	vector_addition(&d_X[idx1*q],d_tempVector[0],&d_P[i3to1(buffsw,idx1,0,n,q)],q,q,1,1);
	...
	for(k = jq-1;k>=1;k--){
		...
		if(k-2>=0){
			cudaMemcpyAsync(&d_Q[i3to1(!buffsw,0,0,n,q)], &Q[i3to1(k-2,0,0,n,q)], n*q*sizeof(my_float), cudaMemcpyHostToDevice,stream[0]);
			cudaMemcpyAsync(&d_P[i3to1(!buffsw,0,0,n,q)], &P[i3to1(k-2,0,0,n,q)], n*q*sizeof(my_float), cudaMemcpyHostToDevice,stream[1]);
		}
		...
		for(j= 1;j<(int)pow(2,jq-k)-1;j++){
			...
			matrixVector_multiplication(d_tempVector[1],d_matrixT,d_tempVector[0],q,q,q,1.0);
			vector_addition(d_tempVector[0],&d_Q[i3to1(buffsw,point,0,n,q)],d_tempVector[1],q,q,1,-1);
			matrixVector_multiplication(d_tempVector[1],d_matrixB,d_tempVector[0],q,q,q,1.0);
			vector_addition(&d_X[point*q],d_tempVector[1],&d_P[i3to1(buffsw,point,0,n,q)],q,q,1,1);
		}
		j = 0;
		point = pow(2,k) -pow(2,k-1)-1;
		matrixVector_multiplication(d_tempVector[0],d_matrixT,&d_X[(int)(pow(2,k)-1)*q],q,q,q,1.0);
		vector_addition(d_tempVector[1],&d_Q[i3to1(buffsw,point,0,n,q)],d_tempVector[0],q,q,1,-1);
		matrixVector_multiplication(d_tempVector[0],d_matrixB,d_tempVector[1],q,q,q,1.0);
		vector_addition(&d_X[point*q],d_tempVector[0],&d_P[i3to1(buffsw,point,0,n,q)],q,q,1,1);
	...
	for(int i=0;i<n*q;i++){
		if(fabs(Xcpu[i] - X[i]) > tol) printf("Files are different! @cpu : %lf - @gpu : %lf position :%ld\n",Xcpu[i],X[i],i);
	}
	fclose (pFile);
	free (Xcpu);
#endif
